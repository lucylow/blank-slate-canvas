...
export default function SonomaRaceResults() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("/demo_data/sonoma_race_results.json");
        if (!response.ok) {
              ...
              <tbody>
                {displayedResults.map((result) => (
                  ...
                  >
                    ...
                    <td className="p-2">
                      <Badge
                        variant={result.status === "Running" ? "default" : "destructive"}
                      >
                        {result.status}
                      </Badge>
