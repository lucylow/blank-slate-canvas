...
import { generateSebringReportPDF } from '@/utils/pdfGenerator';

export const SebringPDFReportGenerator: React.FC = () => {
  const [isGenerating, setIsGenerating] = useState(false);

  const handleGeneratePDF = async () => {
    setIsGenerating(true);
    try {
      await generateSebringReportPDF();
    } catch (error) {
    ...
    >
      {isGenerating ? (
        ...
        <>
          ...
          Download Sebring International Raceway Report PDF
        </>
      )}
    </Button>
  );
