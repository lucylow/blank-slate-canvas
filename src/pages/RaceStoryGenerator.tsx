...
export default function RaceStoryGenerator() {
  ...
  const handleShareHighlight = async (highlight: RaceMoment) => {
    const shareText = `${highlight.title}\n\n${highlight.description}\n\nLap ${highlight.lap} - ${highlight.timestamp}`;
    const shareData = {
    ...
    if (navigator.share && navigator.canShare && navigator.canShare(shareData)) {
      try {
        ...
        handleCopy(shareText, highlight.id);
      }
    } else {
  ...
  const handleExportHighlight = (highlight: RaceMoment) => {
    const exportData = {
      ...highlight,
    ...
    a.href = url;
    a.download = `race-moment-${highlight.id}-lap${highlight.lap}.json`;
  ...
  const handleExportReport = () => {
    const reportData = {
      raceId: selectedRace,
      track: highlights[0]?.title.split(" - ")[1] || "Unknown Track",
      date: new Date().toISOString(),
      highlights: highlights,
      summary: `Race story with ${highlights.length} key moments identified`,
      exportedAt: new Date().toISOString(),
    };
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `race-story-report-${selectedRace}-${new Date().toISOString().split("T")[0]}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  const handleGenerateVisuals = () => {
    // Generate a visual summary text file with all highlights
    const visualSummary = highlights
      .map(
        (h, idx) =>
          `${idx + 1}. ${h.title}\n   ${h.description}\n   Lap ${h.lap} | ${h.timestamp} | Impact: ${h.impact.toUpperCase()}\n`
      )
      .join("\n");
    const blob = new Blob([visualSummary], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `race-story-visuals-${selectedRace}-${new Date().toISOString().split("T")[0]}.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  const handleCreateRecapVideo = () => {
    // Placeholder for video creation - could integrate with video generation service
    alert("Video recap generation is coming soon! This feature will create a highlight reel from the race moments.");
  };

  const handleShareStory = async () => {
    const storyText = `Race Story: ${highlights.length} Key Moments\n\n${highlights
      .map((h) => `• ${h.title} (Lap ${h.lap})`)
      .join("\n")}\n\nGenerated by PitWall AI`;
    const shareData = {
      title: "Race Story Highlights",
      text: storyText,
      url: window.location.href,
    };
    ...
    if (navigator.share && navigator.canShare && navigator.canShare(shareData)) {
      try {
        ...
        // User cancelled or error occurred, fall back to copy
        handleCopy(storyText, "story-share");
      }
    } else {
      // Fall back to copy if Web Share API is not available
      handleCopy(storyText, "story-share");
  ...
  const handleDownloadAll = () => {
    const allData = {
      ...
      summary: `Complete race story export with ${highlights.length} key moments, ${englishSummaries.length} summaries, and ${broadcastCards.length} broadcast cards`,
      exportedAt: new Date().toISOString(),
    };
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `race-story-complete-${selectedRace}-${new Date().toISOString().split("T")[0]}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
                    ...
                    <CardContent className="space-y-5">
                      ...
                      <div className="flex gap-3 pt-2">
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleExportHighlight(highlight);
                          }}
                        >
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-3">
              ...
              <Button variant="outline" onClick={handleGenerateVisuals}>
                <ImageIcon className="h-4 w-4 mr-2" />
                Generate Visuals
              </Button>
              <Button variant="outline" onClick={handleCreateRecapVideo}>
                <Video className="h-4 w-4 mr-2" />
                Create Recap Video
              </Button>
              <Button variant="outline" onClick={handleShareStory}>
